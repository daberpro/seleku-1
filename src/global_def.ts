	...
	return (new Promise((resolve,reject)=>{
		fs.open(file_name,"r",(err: any,data: any): void =>{
			...
			fs.readFile(data,(err: any,_data: any)=>{
				let a: string[] = _data.toString("utf-8").replace(/\<html>/igm,"#html").replace(/\<*\/html>/igm,"#html").replace(/\<style.*?\>/igm,"#css").replace(/\<*\/style>/igm,"#css").replace(/\<script.*?\>/igm,"#js").replace(/\<*\/script>/igm,"#js").replace(/\#head#/igm,"#head").split(/\n/);
...
exports.AST = (args: Array<final_data>, lex: string[][],css: string[],js: string[],head: string[],config: any)=>{
	...
	let convert_others = (argv:any,something: any)=>{
	...
	let $head_data_parser : string = "";
	head.forEach($child =>{
		$head_data_parser += $child.trim().replace("#head","");
	});
	let $head_data : any = {...nodeParser.parse($head_data_parser)};
	let $head_result: any = [];
	convert_others({
